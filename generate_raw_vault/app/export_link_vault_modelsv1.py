...
#     link_files = [
#         Path(file).name for file in glob.iglob(f"models/raw_Vault/links/*.sql", recursive=True)
#     ]
#     print(link_files)
...
#     create_link_files(metadata_file_dirs)
...
def create_link_files(metadata_file_dirs):
    template = load_template_file("generate_raw_vault/app/templates/link_model.sql")
    link_template = Template(template)
    table_name = []
    for metadata_file_path in metadata_file_dirs:
        ...
        metadata_file = load_metadata_file(metadata_file_path)
        metadata = Metadata(metadata_file)
        table_name.append(metadata.get_versioned_source_name().lower())
        ...
        for v in itertools.combinations(sorted(hubs), 2):
            ...
            # for hub in hubs:
            substitutions = create_link_subsitutions(metadata, v, table_name)
...
def create_link_subsitutions(metadata, hubs, table_name):
    # database_name = metadata.get_target_database()
    # schema_name = metadata.get_target_schema()
    # source_name = f"{database_name}_{schema_name}"
