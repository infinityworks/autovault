...
class BusinessTopic:
    business_keys: Dict[str, str]
    business_attributes: List[BusinessAttribute]
    alias: Optional[str] = None

@dataclass_json
@dataclass
class DataValutConfig:
    unit_of_work: str
    source_name: str
    version: str
    source_system: str
...
class Metadata:
    def __init__(self, metadata: dict):
        self.metadata: DataValutConfig = DataValutConfig.from_dict(metadata)

    def get_source_name(self):
        return self.metadata.source_name.replace("-", "_")

    def get_unit_of_work(self):
        return self.metadata.unit_of_work.replace("-", "_")

    def get_source_system(self):
        return self.metadata.source_system.replace("-", "_")

    def get_target_schema(self):
        return self.metadata.destination_schema.replace("-", "_")

    def get_target_database(self):
        return self.metadata.destination_database.replace("-", "_")

    def get_source_version(self):
        return self.metadata.version

    def get_business_topics(self):
        return self.metadata.business_topics

    def get_versioned_source_name(self):
        return f"{self.get_source_name()}_V{self.get_source_version()}"
    ...
    def get_hub_alias(self, hub):
        ...
        hub_alias = topics.get(hub).alias # TODO: this looks wrong
        return hub_alias
    ...
    def get_sat_from_hub(self, hub):
        ...
        sats = {
            topic.business_definition : topic.payload
            for topic in topics.business_attributes
        }
        return sats
    ...
    def get_topic_key(self, topic, business_topics: Dict[str, BusinessTopic]):
        return {
            "natural_keys": business_topics.get(topic).business_keys,
            "alias": business_topics.get(topic).alias,
        }
    ...
    def get_attributes(self, attr: BusinessAttribute):
        if "null" in attr.payload.keys(): # TODO: I'm not entirely sure why this is here ...
            del attr.payload["null"]
        source_attributes = [{key: value} for key, value in attr.payload.items()]
        return source_attributes

    def flatten_business_attributes(self):
        topics = self.get_source_business_topics()
        business_attributes = [topic.business_attributes for topic in topics]
